...
function xoa_loai_qua()
{
	...
	//kiem tra xem loai cay co lien quan den 
	$strSQL="SELECT COUNT(*) FROM cay WHERE ma_loai={$maloaiqua}";
	...
	$strSQL="DELETE FROM loai_cay WHERE ma_loai={$maloaiqua}";
...
function them_loai_qua()
{
	...
	//kiem tra loai cay co trung ten voi loai cay da co hay ko
		$strSQL="SELECT COUNT(*) FROM loai_cay WHERE ten_loai ='{$tenloaiqua}'";
	...
	//neu khong trung ten luu vao csdl
		$strSQL="INSERT INTO loai_cay(ten_loai) VALUES('{$tenloaiqua}')";
		mysqli_query($ung,$strSQL);
...
{	
	...
	//neu khong trung ten luu vao csdl
		...
		$strSQL="UPDATE loai_cay SET ten_loai='{$tenloaiqua}' WHERE ma_loai={$maloaiqua}";
		mysqli_query($ung,$strSQL);
